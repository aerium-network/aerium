...
cp ${BUILD_DIR}/aerium-gui                ${GUI_BUNDLE}
cp ${ROOT_DIR}/.github/releasers/macos/*  ${GUI_BUNDLE}

# Icon
cp ${ROOT_DIR}/.github/releasers/aerium.icns  ${GUI_BUNDLE}
...
rm -rf ${ROOT_DIR}/aerium-gui.app/Contents/Resources/Cellar
...
create-dmg --skip-jenkins \
  --volname "Aerium GUI" \
  "${FILE_NAME}.dmg" \
  "${ROOT_DIR}/aerium-gui.app"

echo "Creating archive"
cp ${BUILD_DIR}/aerium-daemon     ${PACKAGE_DIR}
cp ${BUILD_DIR}/aerium-wallet     ${PACKAGE_DIR}
cp ${BUILD_DIR}/aerium-shell      ${PACKAGE_DIR}
cp -R ${ROOT_DIR}/aerium-gui.app  ${PACKAGE_DIR}
