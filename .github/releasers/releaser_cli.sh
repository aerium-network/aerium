...
for OS_ARCH in \
    ...
    echo "Building Aerium for ${OS}-${ARCH}..."

    LD_FLAGS="-s -w"
    if [[ ${OS} == "android" ]]; then
        LD_FLAGS="${LD_FLAGS} -checklinkname=0"
    fi
    CGO_ENABLED=0 GOOS=${OS} GOARCH=${ARCH} go build -ldflags "${LD_FLAGS}" -trimpath -o ${BUILD_DIR}/${PACKAGE_NAME}/aerium-daemon${EXE} ./cmd/daemon
    CGO_ENABLED=0 GOOS=${OS} GOARCH=${ARCH} go build -ldflags "${LD_FLAGS}" -trimpath -o ${BUILD_DIR}/${PACKAGE_NAME}/aerium-wallet${EXE} ./cmd/wallet
    CGO_ENABLED=0 GOOS=${OS} GOARCH=${ARCH} go build -ldflags "${LD_FLAGS}" -trimpath -o ${BUILD_DIR}/${PACKAGE_NAME}/aerium-shell${EXE}  ./cmd/shell
