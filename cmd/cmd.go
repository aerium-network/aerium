...
func AeriumDefaultWalletPath(home string) string {
	return filepath.Join(AeriumWalletDir(home), DefaultWalletName)
}
...
func StartNode(workingDir string, passwordFetcher func(*wallet.Wallet) (string, bool),
	...
	defaultWalletPath := AeriumDefaultWalletPath(workingDir)
...
func MakeConfig(workingDir string) (*config.Config, *genesis.Genesis, error) {
	gen, err := genesis.LoadFromFile(AeriumGenesisPath(workingDir))
	if err != nil {
		return nil, nil, err
	}
	...
	walletsDir := AeriumWalletDir(workingDir)
	confPath := AeriumConfigPath(workingDir)
