...
func TestBasicCheck(t *testing.T) {
	...
	t.Run("Invalid amount", func(t *testing.T) {
		...
		err := trx.BasicCheck()
		assert.ErrorIs(t, err, tx.BasicCheckError{
			Reason: "invalid amount: -0.000000001 AUM",
		})
	})
	...
	t.Run("Invalid amount", func(t *testing.T) {
		...
		err := trx.BasicCheck()
		assert.ErrorIs(t, err, tx.BasicCheckError{
			Reason: "invalid amount: 42000000 AUM",
		})
	})
...
func TestSignBytesBLS(t *testing.T) {
	...
	assert.Equal(t, "ae1zyyynyjnsck7h5pv3hnvdt97t8gr2j9msrx8tfu", trx.Payload().Signer().String())
}
...
func TestSignBytesEd25519(t *testing.T) {
	...
	assert.Equal(t, amount.Amount(1000), trx.Fee())
	assert.Equal(t, amount.Amount(20000), trx.Payload().Value())
	assert.Equal(t, "ae1rwzvr8rstdqypr80ag3t6hqrtnss9nwym5vqehp", trx.Payload().Signer().String())
}
...
func TestCheckFee(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedErr: tx.BasicCheckError{Reason: "invalid fee: -0.000000001 AUM"},
		},
		{
			name: "Big fee",
			trx: ts.GenerateTestTransferTx(
				testsuite.TransactionWithFee(42e15 + 1)),
			expectedErr: tx.BasicCheckError{Reason: "invalid fee: 42000000 AUM"},
		},
		{
