...
func (s *Server) getOpenAPIHandler() (http.Handler, error) {
	...
	origContent, err := swaggerFS.ReadFile("swagger-ui/aerium.swagger.json")
	if err != nil {
		return nil, err
	}
...
func (s *Server) StartServer(grpcAddr string) error {
	...
	if err := aerium.RegisterBlockchainHandler(s.ctx, gatewayMux, grpcConn); err != nil {
		return err
	}
	if err := aerium.RegisterTransactionHandler(s.ctx, gatewayMux, grpcConn); err != nil {
		return err
	}
	if err := aerium.RegisterNetworkHandler(s.ctx, gatewayMux, grpcConn); err != nil {
		return err
	}
	if err := aerium.RegisterWalletHandler(s.ctx, gatewayMux, grpcConn); err != nil {
		return err
	}
	if err := aerium.RegisterUtilsHandler(s.ctx, gatewayMux, grpcConn); err != nil {
		return err
	}
...
func (*Server) changeBasePath(handler http.Handler, modifiedContent []byte) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/aerium.swagger.json" {
