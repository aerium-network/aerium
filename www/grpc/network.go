...
func (s *networkServer) GetNodeInfo(_ context.Context,
	_ *aerium.GetNodeInfoRequest,
) (*aerium.GetNodeInfoResponse, error) {
	...
	resp := &aerium.GetNodeInfoResponse{
		...
		ConnectionInfo: &aerium.ConnectionInfo{
		...
		ZmqPublishers: make([]*aerium.ZMQPublisherInfo, 0),
	}

	for _, publisher := range s.zmqPublishers {
		resp.ZmqPublishers = append(resp.ZmqPublishers, &aerium.ZMQPublisherInfo{
...
func (s *networkServer) GetNetworkInfo(_ context.Context,
	req *aerium.GetNetworkInfoRequest,
) (*aerium.GetNetworkInfoResponse, error) {
	peerSet := s.sync.PeerSet()
	peerInfos := make([]*aerium.PeerInfo, 0, peerSet.Len())
	...
	peerSet.IteratePeers(func(peer *peer.Peer) bool {
		...
		peerInfo.Direction = aerium.Direction(peer.Direction)
	...
	return &aerium.GetNetworkInfoResponse{
...
func metricToProto(metric metric.Metric) *aerium.MetricInfo {
	metricInfo := &aerium.MetricInfo{
		TotalInvalid: &aerium.CounterInfo{
			Bytes:   uint64(metric.TotalInvalid.Bytes),
			Bundles: uint64(metric.TotalInvalid.Bundles),
		},

		TotalSent: &aerium.CounterInfo{
			Bytes:   uint64(metric.TotalSent.Bytes),
			Bundles: uint64(metric.TotalSent.Bundles),
		},

		TotalReceived: &aerium.CounterInfo{
			Bytes:   uint64(metric.TotalReceived.Bytes),
			Bundles: uint64(metric.TotalReceived.Bundles),
		},
	}

	metricInfo.MessageSent = make(map[int32]*aerium.CounterInfo)
	for msgType, counter := range metric.MessageSent {
		metricInfo.MessageSent[int32(msgType)] = &aerium.CounterInfo{
			Bytes:   uint64(counter.Bytes),
			Bundles: uint64(counter.Bundles),
		}
	}

	metricInfo.MessageReceived = make(map[int32]*aerium.CounterInfo)
	for msgType, counter := range metric.MessageReceived {
		metricInfo.MessageReceived[int32(msgType)] = &aerium.CounterInfo{
			Bytes:   uint64(counter.Bytes),
			Bundles: uint64(counter.Bundles),
		}
	}
