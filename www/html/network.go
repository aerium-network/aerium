...
func metricToTable(tmk *tableMaker, metricInfo *aerium.MetricInfo) {
	printCounter := func(tmk *tableMaker, name string, counterInfo *aerium.CounterInfo) {
		tmk.addRowString(name,
			fmt.Sprintf("[%d, %s]", counterInfo.Bundles, util.FormatBytesToHumanReadable(counterInfo.Bytes)))
	}

	printSortedMap := func(tmk *tableMaker, msgCounter map[int32]*aerium.CounterInfo) {
