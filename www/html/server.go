...
type Server struct {
	...
	blockchain  aerium.BlockchainClient
	transaction aerium.TransactionClient
	network     aerium.NetworkClient
	router      *mux.Router
	enableAuth  bool
...
func (s *Server) StartServer(grpcServer string) error {
	...
	s.grpcConn = grpcConn
	s.blockchain = aerium.NewBlockchainClient(grpcConn)
	s.transaction = aerium.NewTransactionClient(grpcConn)
	s.network = aerium.NewNetworkClient(grpcConn)
