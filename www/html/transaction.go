...
func txToTable(tmk *tableMaker, trx *aerium.TransactionInfo) {
	if trx == nil {
		return
	}
	...
	case aerium.PayloadType_PAYLOAD_TYPE_TRANSFER:
		pld := trx.Payload.(*aerium.TransactionInfo_Transfer).Transfer
	...
	case aerium.PayloadType_PAYLOAD_TYPE_BOND:
		pld := trx.Payload.(*aerium.TransactionInfo_Bond).Bond
		tmk.addRowAccAddress("Sender", pld.Sender)
		tmk.addRowValAddress("Receiver", pld.Receiver)
		tmk.addRowAmount("Stake", amount.Amount(pld.Stake))

	case aerium.PayloadType_PAYLOAD_TYPE_SORTITION:
		pld := trx.Payload.(*aerium.TransactionInfo_Sortition).Sortition
		tmk.addRowValAddress("Address", pld.Address)
		tmk.addRowString("Proof", pld.Proof)

	case aerium.PayloadType_PAYLOAD_TYPE_UNBOND:
		pld := trx.Payload.(*aerium.TransactionInfo_Unbond).Unbond
		tmk.addRowValAddress("Validator", pld.Validator)

	case aerium.PayloadType_PAYLOAD_TYPE_WITHDRAW:
		pld := trx.Payload.(*aerium.TransactionInfo_Withdraw).Withdraw
		tmk.addRowValAddress("Sender", pld.ValidatorAddress)
		tmk.addRowAccAddress("Receiver", pld.AccountAddress)
		tmk.addRowAmount("Amount", amount.Amount(pld.Amount))

	case aerium.PayloadType_PAYLOAD_TYPE_BATCH_TRANSFER:
		pld := trx.Payload.(*aerium.TransactionInfo_BatchTransfer)
	...
	case aerium.PayloadType_PAYLOAD_TYPE_UNSPECIFIED:
